module.exports = function(logger){
     ...
     * @param orgName
     */
    join_channel.peerJoinChannel = function(channelName, peers, userName, orgName){
        ...
        try{
            ...
            helper.getClientForOrg(orgName, userName).then((client)=>{
                //获取指定的channel对象
                ...
                //获取genesis_block
                let genesis_block = channel.getGenesisBlock(request);
                        ...
                        let block_registration_number = eventHubs.registerBlockEvent((block)=>{
                            ...
                            if (block.data.data.length === 1) {
                                ...
                                if (channel_id === channelName) {
                                    let message = util.format('EventHub % has reported a block update for channel %s',eventHubs._ep._endpoint.addr,channelName);
                                    logger.info('[FzuChain]: '+message);
                                    resolve(message);
                                } else {
                ...
                logger.error('[FzuChain]: Failed to get client for org due to error: ' + error.stack ? error.stack : error);
            });
